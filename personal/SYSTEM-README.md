# `personal/` — personal (IndieWeb) context
...
**Use this variant** for a new or lightly-documented repo, where a clean install is what you want. If the repo already has agentic markdown you'd rather adapt than sit beside, use `personal_starter/` instead.
...
installed. Do not copy `ADAPTATION.md` — it explains why this context differs from the other, and has no job in your repo.
...
GRAPH-AGENTS.md      Multi-service process — delete it if you have one service
CLAUDE.md            Pointer to AGENTS.md. No unique rules.
GEMINI.md            Pointer to AGENTS.md. No unique rules.
MEMORY.md            Confirmed durable lessons          ┐
DECISIONS.md         Architectural choices, checkpoints  │ headers only —
CONSTRAINTS.md       Binding constraints                 │ filled in during work
DESIGN.md            Aesthetic references, taste        ┘
ADAPTATION.md        Reference only. Do not copy this one across.
...
```
personal/.claude/skills/gallery-format/  →  <your repo>/.claude/skills/gallery-format/
personal/.agents/skills/gallery-format/  →  <your repo>/.agents/skills/gallery-format/
```
...
`LOOP-AGENTS.md` and delete `GRAPH-AGENTS.md`. More than one repo or deploy
...
### 3. Paste a setup prompt

Copying the files in is already a working install. Run a prompt when you want
the model roster populated, or when your repo already had documentation that
needs reconciling rather than sitting next to the new files.

Open a **fresh session** in your agent — not one already mid-task — and paste
one of the prompts at the end of this SYSTEM-README:

| Situation | Paste this |
|---|---|
| One codebase, one deploy target | Prompt A — Single-Service |
| Several repos or deploy targets coordinated together | Prompt B — Multi-Service |

Both are idempotent. Running one twice does not double up.
...
Do not create an `ADAPTATION.md` in the target repo. It is reference material
explaining why this context differs from the canonical framework; the
differences are already baked into the files above.
