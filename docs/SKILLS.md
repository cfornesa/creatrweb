...
Skills are loadable instruction sets that extend AGENTS.md for specific tasks. They live in \`.agents/skills/\` and \`.claude/skills/\` as byte-identical mirrors, shipped inside each context folder (\`general/\` or \`personal/\`) and installed at your repo root. Load them on demand or let the rules load them automatically.
