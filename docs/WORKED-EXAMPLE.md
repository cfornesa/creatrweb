...
## Reading this after the restructure

This build ran in April 2026, before the framework was reorganized into two
self-contained context payloads. Nothing in the narrative below is wrong, but
the file names have moved. Translating it to the current structure:

| What the narrative says | Where it lives now |
|---|---|
| "AGENTS.md" (the profile copy) | Split in two: `personal/AGENTS.md` (rules and routing) and `personal/LOOP-AGENTS.md` (Merge Protocol, loop, roster, Irreversible Decisions) |
| The IndieWeb rules that fired here | Baked into `personal/AGENTS.md` (Rule 5, Core Constraints, the skill table) and `personal/LOOP-AGENTS.md` (the Irreversible Decisions table) |
| `posse-syndication`, `security` skills | `personal/.claude/skills/` and its `.agents/` mirror |
| `gallery-format`, `design-workflow`, `memory-files` | Same folder — they ship in both contexts |
| `MEMORY.md`, `DECISIONS.md`, `DESIGN.md` | Unchanged — still per-project working files at the target repo's root |
| The `EVAL_PROMPT.md` pass | Now `AGENTS.md` Section 12, Post-Session Eval |
| The Plan Mode rule added to `CLAUDE.md` | Now `AGENTS.md` Section 4 — `CLAUDE.md` is a pure pointer with no rules of its own |

Because creatrweb.com is an IndieWeb site, this build would today install the
`personal/` payload. Rules 2 and 6 — which drive most of this narrative — are
identical in both contexts and would have fired the same way under either.

The repo was a Case A repo at the time: no markdown, framework installed from
scratch. A rebuild today would be Case E — a mature, Creatrweb-lineage
documentation network that gets adopted, not duplicated.
